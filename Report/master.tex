...
%an overview will be given. or "the implementation will be ..."
...
the window offset can be very small (as small as possible without losing the peaks). Otherwise you will merge too many peaks using the peakfinder. 0.5 of even 0.1 should be ok. \todo{todo}
...
%yes. put a couple of examples, and possibly more in the appendix.


%Or operating on both channels at the same time.
%
%AX11 and AX12 as one defect crossing sample with two signatures seen from different location. This would help in cases where the defect is only on one side of the track.
%
%Add this, and keep this in mind as a limitation of the current approach
...
%		I'm not an expert on neural networks. If anything uncertain here. I'd ask Harry.
		\vspace*{0.25cm}\\
		\hline
...
%yes!! Very important.
%
%And also be aware that even if the model performs badly it is ok. However no cheating allowed to get good numbers ;)  
%
%60-70% Auc would be ok.
%I'll test your code on the new samples. We'll do like the machine learning class, where the baseline result will be the accuracy delivered by a simple regression or decision tree ;););)
...
\begin{itemize}
	...
	%	no X. But sensor fusion 
	%X+Y * 4 = 8 Channels
	%
	%Also Y will be very clearly different at Switches (lots of sideway hits, accelerations).
	%
	%Or additional sensors higher up in the vehicle! Bogie(Z,Y), Body (Y)
	\item range defects
...
%Would be cool to continue on this, or meetup every now and then in Europe or Singapore (once T-Virus epidemy is over)
...
numpy array is faster for operating on data.
Pandas s more convenient for storing the data

%here there are many points that you can mention that can be done and that the library code is built in an extentible form
